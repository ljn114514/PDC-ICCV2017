...
#include "caffe/layers/new_facial_point_data_layer.hpp"
...
template <typename Dtype>
NewFacialPointDataLayer<Dtype>::~NewFacialPointDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void NewFacialPointDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int new_height = this->layer_param_.new_facial_point_data_param().new_height();
  const int new_width  = this->layer_param_.new_facial_point_data_param().new_width();
  const bool is_color  = this->layer_param_.new_facial_point_data_param().is_color();
  string root_folder = this->layer_param_.new_facial_point_data_param().root_folder();
  const int point_num  = this->layer_param_.new_facial_point_data_param().point_num();
  const bool use_face_rect = this->layer_param_.new_facial_point_data_param().use_face_rect();
  const float xy_mean = this->layer_param_.new_facial_point_data_param().xy_mean();
  const float xy_std = this->layer_param_.new_facial_point_data_param().xy_std();
  const float wh_mean = this->layer_param_.new_facial_point_data_param().wh_mean();
  const float wh_std = this->layer_param_.new_facial_point_data_param().wh_std();

  CHECK((new_height > 0 && new_width > 0)) << "Current implementation requires "
      "new_height and new_width to be set at the same time.";

  const string& source = this->layer_param_.new_facial_point_data_param().source();
  ...
  if (this->layer_param_.new_facial_point_data_param().shuffle()) {
...
// Read an image, and use it to initialize the top blob.
  ...
  const int batch_size = this->layer_param_.new_facial_point_data_param().batch_size();
...
template <typename Dtype>
void NewFacialPointDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void NewFacialPointDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  NewFacialPointDataParameter facial_point_data_param = this->layer_param_.new_facial_point_data_param();
...
INSTANTIATE_CLASS(NewFacialPointDataLayer);
REGISTER_LAYER_CLASS(NewFacialPointData);
