...
imglist = './VGG_STN_300W/test_list_off_det.txt'; %
model_file = './VGG_STN_300W/facial_point_iter_30000.caffemodel';
model_def_file = './VGG_STN_300W/deploy.prototxt';
...
%             text(ground_truth(n, 1, j) - 0.02, ground_truth(n, 1, j), num2str(n), 'color', 'g');
    ...
    rectangle('Position', [rct(1), rct(2), rct(3) - rct(1), rct(4) - rct(2)], 'EdgeColor', 'r');
...
%     input_data = {images, label};
    ...
    theta = squeeze(theta)';
    st_label = net.blobs('local/68point').get_data();
    st_label = squeeze(st_label)';
    st_data = net.blobs('local/data').get_data();
...
%             text(ground_truth(n, 1, j) - 0.02, ground_truth(n, 1, j), num2str(n), 'color', 'g');
    ...
    rectangle('Position', [global_rct(1), global_rct(2), global_rct(3) - global_rct(1), global_rct(4) - global_rct(2)], 'EdgeColor', 'r');
