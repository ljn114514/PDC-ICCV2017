...
model_file = fullfile(parent_path, 'facial_point_iter_63151.caffemodel');
model_def_file = fullfile(parent_path, 'deploy.prototxt');
mat_result_path = fullfile(result_path, 'VGG-2-det.mat');
...
% 	h = w;
	...
	%% second stage
    pts = reshape(pts,pts_num,[]);
    rct = [min(pts) max(pts)];
    
    w = rct(3) - rct(1);
	h = rct(4) - rct(2);
	rct(1) = floor(rct(1) - ext_scale_2*w/2);
	rct(2) = floor(rct(2) - ext_scale_2*h/2);
	rct(3) = floor(rct(3) + ext_scale_2*w/2);
	rct(4) = floor(rct(4) + ext_scale_2*w/2);
    if rct(1)<=0
        rct(1) = 1;
    end
    if rct(2)<=0
